...
func TestMultiStoreQuery(t *testing.T) {
	...
	assert.Equal(t, abciapp.ToABCICode(abciapp.CodespaceRoot, abciapp.CodeUnknownRequest), abciapp.ABCICodeType(qres.Code))
	...
	qres = multi.Query(query)
	assert.Equal(t, abciapp.ToABCICode(abciapp.CodespaceRoot, abciapp.CodeOK), abciapp.ABCICodeType(qres.Code))
...
func newMultiStoreWithMounts(db dbm.DB) *rootMultiStore {
	...
	store.MountStoreWithDB(
		abciapp.NewKVStoreKey("store1"), abciapp.StoreTypeIAVL, db)
	store.MountStoreWithDB(
		abciapp.NewKVStoreKey("store2"), abciapp.StoreTypeIAVL, db)
	store.MountStoreWithDB(
		abciapp.NewKVStoreKey("store3"), abciapp.StoreTypeIAVL, db)
	return store
}
